...
<header class="bg-gradient-to-r from-pink-500 via-purple-500 to-teal-500 shadow fixed top-0 w-full z-20" x-data="{ open: false }">
    ...
    <div class="md:hidden" x-show="open" @click.away="open = false">
        <nav class="bg-white px-6 pb-4 space-y-4">
            <a href="#how-it-works" class="block text-gray-700 hover:text-pink-500 font-medium">{{ __('messages.nav_how_it_works') }}</a>
            <a href="#features" class="block text-gray-700 hover:text-pink-500 font-medium">{{ __('messages.nav_features') }}</a>
            <a href="#why-us" class="block text-gray-700 hover:text-pink-500 font-medium">{{ __('messages.nav_why_us') }}</a>
            <a href="#contact" class="block text-gray-700 hover:text-pink-500 font-medium">{{ __('messages.nav_demo') }}</a>
            <div class="flex items-center space-x-2 pt-2 border-t">
                <a href="{{ route('language.switch', ['locale' => 'en']) }}" class="text-gray-700 hover:text-pink-500 font-medium {{ app()->getLocale() == 'en' ? 'font-bold' : '' }}">EN</a>
                <span class="text-gray-500">|</span>
                <a href="{{ route('language.switch', ['locale' => 'de']) }}" class="text-gray-700 hover:text-pink-500 font-medium {{ app()->getLocale() == 'de' ? 'font-bold' : '' }}">DE</a>
            </div>
        </nav>
    </div>
</header>
